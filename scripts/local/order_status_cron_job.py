...
def add_driver_information(orders_ids, py_sql):
    for order_id in orders_ids:
        ...
        (first_name, last_name, vehicle_model, vehicle_number, contact_no, created_date, updated_date, order_id)
        VALUES(%s, %s, %s, %s, %s, %s, %s, %s)
        '''
        py_sql.params = [
            'Arslan', 'Sherazi', 'United CD-70', 'LEP-5122', '03336664142', datetime.now(), datetime.now(), order_id
        ]
        py_sql.save_changes()
...
if __name__ == '__main__':
    ...
    schedule.every().minute.at(':10').do(handle_placed_order_status, py_sql)
    schedule.every().minute.at(':20').do(handle_accepted_order_status, py_sql)
    schedule.every().minute.at(':30').do(handle_under_preparation_order_status, py_sql)
    schedule.every().minute.at(':59').do(handle_on_route_order_status, py_sql)
