...
from apps.user.apis.v100.login.validator import LoginValidator
...
class Login(BasePostResource):
    ...
    def generate_authentication_token(self):
        ...
        response = requests.post(settings.API_BASE_URL.format('/v100/api_token'), data=params, auth=authentication)
