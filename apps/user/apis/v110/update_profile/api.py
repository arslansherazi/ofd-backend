...
from apps.user.apis.v110.update_profile.validator import UpdateProfileValidator
from apps.user.models_v.v110.user import UserV110
from apps.user.repositories.v100.user_repo import UserRepository
from apps.user.repositories.v110.user_repo import UserRepositoryV110
...
class UpdateProfile(BasePostResource):
    ...
    def process_profile_image(self):
        ...
        """
        old_profile_image_url = UserV110.get_profile_image_url(self.user_id)
    ...
    def check_username_availability(self):
        if self.username:
            is_username_available = UserV110.check_username_availability(self.username, self.user_type)
            if not is_username_available:
                self.is_send_response = True
                self.status_code = 422
                self.response = {
                    'message': UserRepositoryV110.USERNAME_ALREADY_EXIST
                }
    ...
    def update_profile(self):
        ...
        if self.profile_image_url or self.username:
            UserV110.update_data(self.user_id, self.username, self.profile_image_url)
        if self.user_type == BUYER_USER_TYPE:
    ...
    def prepare_response(self):
        ...
        if self.username:
            self.response['data'].update(message=UserRepositoryV110.USERNAME_UPDTED_SUCCESSFULLY)
